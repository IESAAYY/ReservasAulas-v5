	...
	void entrar(ActionEvent event) {
		try {
			...
			pt.setOnFinished(ef -> {
				...
				FXMLLoader loader = new FXMLLoader(LocalizadorRecursos.class.getResource("vistas/Marco.fxml"));
				Parent root;
				try {
					root = loader.load();
					...
					/*
					 * root.setOnMousePressed((EventHandler<? super MouseEvent>) new
					 * EventHandler<MouseEvent>() {
					 * 
					 * @Override public void handle(MouseEvent event) { xOffset = event.getSceneX();
					 * yOffset = event.getSceneY(); } }); root.setOnMouseDragged(new
					 * EventHandler<MouseEvent>() {
					 * 
